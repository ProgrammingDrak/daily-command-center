...
test("an unwritten Journal does NOT inflate the Loose Ends pill", async () => {
  ...
  assert.equal(pill.hidden, true, "nothing is actually outstanding");
});

test("an unwritten Journal still opens the morning prompt once", async () => {
...
test("fresh sweep arrivals still open when the Journal scope is temporarily unavailable", async () => {
