...
function createSlackActors({ pool, env = process.env, apiTimeoutMs = 20_000 } = {}) {
  ...
  function makeActor({ userId, workspaceId, slackUserId, slackTeamId, slackHost, userToken, linkedVia }) {
  ...
  function actorFromRow(row) {
    return makeActor({
      ...
      // Tier 2 will decrypt row.user_token_enc here. Until then every stored
      // identity is bot tier, which is why it is not even selected.
      userToken: "",
  ...
  async function findRow(slackUserId) {
    ...
    const { rows } = await pool.query(
      `SELECT slack_user_id, slack_team_id, user_id, workspace_id, slack_host, linked_via
  ...
  async function resolveActor(slackUserId, teamId) {
    ...
    // 1. Env fallback, before any DB work.
    ...
    if (fresh && hit.actor) return hit.actor;

    try {
      let row = await findRow(slackUserId);
      // The reaction IS the proof: a pending claim becomes a real link here, and
      // the same reaction goes on to create its task.
      if (row && row.linked_via === PENDING) row = await activatePending(row);
      if (row) {
