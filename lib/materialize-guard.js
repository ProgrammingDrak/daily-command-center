...
// dedupeStatus) and the keyed lookup for the three routes/dcc.js writers.
//
// Three BULK variants remain hand-rolled, because each indexes a whole day's rows once
// and answers many candidates from it — a shape this single-row API would turn into one
// day-load per candidate:
...
// Unifying their LOOKUP wants a bulk `indexForDedupe(rows, keyOf)` helper, which is a
// Track D file change and is not this phase.
//
// And ONE single-row holdout, deliberately not folded: routes/slack-events.js
// `findTaskByKey` runs the same keyed, tombstone-inclusive, live-first query — but it
// breaks ties `created_at DESC` where db.findByIdempotencyKey uses ASC, so on a key with
// several live rows the two return DIFFERENT rows. Reconciling that tie-break is the
// prerequisite to routing it through findForDedupe, and it is Track E's file.
//
// Do not read "one guard" as "grep is unnecessary".
...
function createMaterializeGuard({ blockDB } = {}) {
  ...
  async function findForDedupe(workspaceId, opts = {}) {
    ...
    if (idempotencyKey) {
      ...
      // its own cross-day gate. The other key vocabularies in use —
      // `resp:<id>:<date>`, `resp-shell:<id>:<date>`, and
      // A3's `tg:<groupId>:<date>:<itemIndex>` — all embed the date, so they cannot
      // collide across days. A3's UNIQUE index over live idempotency keys enforces this
      // same global-identity reading.
      //
      // A3 also introduced a genuinely new CATEGORY, which is the part worth writing
      // down: `mcp-schedule:<uuid>` and `cli-schedule:<uuid>` are RETRY-SCOPED nonces,
      // not deterministic entity keys. They are minted once per invocation, before the
      // caller's retry loop, so every attempt at one request shares a key while two
      // deliberate invocations never do. Date-blindness is safe for them for a different
      // reason than the others — they embed no date because they are globally unique,
      // not because they are per-day. Keep this list current: it is the evidence for the
      // date-blind decision above, not decoration.
    ...
    // Pre-loaded rows are scoped here too. Both other branches enforce tenancy (the key
    // query via `workspace_id IS NOT DISTINCT FROM $2`, the date query by passing it
    // down), and a module billed as the tenant-safe home for this rule should not have
    // one branch where tenancy is the caller's problem.
    const candidates = rows
      ? rows.filter((row) => (row.workspace_id || null) === (workspaceId || null))
      : await blockDB.getBlocksByDateIncludingDeleted(date, workspaceId);
