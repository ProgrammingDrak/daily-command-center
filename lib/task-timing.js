...
function createTaskTiming({ pool, blockDB, timeZone = "America/New_York" } = {}) {
  ...
  async function reallocateTimeEntry({ entry, allocations, actor = "dcc", userId = null, workspaceId = null, actionId = null }) {
    ...
    if ((entry.properties || {}).reallocationOperationId === actionId && actionId) {
    ...
    const ownsTransaction = pool && typeof pool.connect === "function";
    ...
    try {
      ...
      const locked = typeof blockDB.getBlockIncludingDeleted === "function"
        ...
        : entry;
      if (!locked || locked.deleted_at) throw new Error("Tracked time not found");
      const props = locked.properties || {};
      if (actionId && props.reallocationOperationId === actionId) {
        if (ownsTransaction) await client.query("COMMIT");
        return { entries: [locked], tasks: [], originTaskId: String(props.blockId || "") || null, sourceEntryDeleted: false, duplicate: true };
      }
      ...
      for (const [id, known] of touched) {
        const block = known || await blockDB.getBlock(id, client);
