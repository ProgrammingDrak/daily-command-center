...
function entryWindow(entry, timeZone = "America/New_York") {
  ...
  const declared = Math.max(0, Math.round(Number(props.durSec) || 0));
...
function planAllocations(totalSec, requests, { maxParts = MAX_ALLOCATION_PARTS } = {}) {
  ...
  if (!(total > 0)) return { ok: false, error: "This segment has no tracked time to move" };
...
function createTaskTiming({ pool, blockDB, timeZone = "America/New_York" } = {}) {
  ...
  async function getSessions(block, opts = {}) {
    if (!block || !block.id) return [];
    if (typeof blockDB.getTaskTimeEntries === "function") {
      return blockDB.getTaskTimeEntries(block.id, block.workspace_id || opts.workspaceId || null, opts);
    }
  ...
  async function recomputeActualMinutes({ block, workspaceId = null, priorSessionsTotal, excludeEntryIds = null }) {
    if (!block || !block.id) return null;
    let sessions = await getSessions(block, { workspaceId });
    // A row that is about to be deleted must not count toward the projection. Without
    // this the ordering is over-constrained: the source of a WHOLE move is still live
    // while the projections run (it has to be, because db.updateBlock refuses a
    // tombstone and the settled marker has to land on a live row), so the origin would
    // re-project the very 60 minutes that just left it. Excluding it by id lets the
    // projection be correct wherever the delete happens to sit.
    if (excludeEntryIds && excludeEntryIds.size) sessions = sessions.filter(row => !excludeEntryIds.has(row.id));
  ...
  // THE ONE MOVER for tracked time. `allocations` is [{ durSec, task }] already
  // planned by planAllocations and resolved by the route, in the order the pieces
  // sit on the clock: piece one starts where the segment started, piece two picks
  // up where piece one ended, and so on. That ordering is the whole UX ("the
  // first 20m was onboarding, the rest was the migration") and it is why the
  // caller must not reorder them.
  //
  // Exactly one row may KEEP the source id: the piece that lands back on the
  // origin task. Every other piece is a new row at a deterministic id, and the
  // source row is deleted when no piece stayed. Two reasons it works this way,
  // and both are bugs if you flip it:
  //
  //  1. A moved piece must not inherit an id that encodes the OLD task. The
  //     Slack timer row is literally `<taskId>-slacktimer`, and deleteTimerRow
  //     hard-deletes it by that id, so a moved piece keeping it would be erased
  //     later by a clearTiming on a task it no longer belongs to.
  //  2. Deterministic ids make a double-submit safe: a replay lands on the same
  //     rows instead of inserting new ones.
  //
  // WRITE ORDER IS A SAFETY PROPERTY, NOT A STYLE CHOICE. There is no transaction
  // here (no function in this module takes a pool client), so a mid-sequence
  // failure is possible and the only question is which direction it fails in.
  // Shrinking the source FIRST makes a failure LOSSY: source at 20m, destination
  // never written, and the minutes are simply gone. So every other piece is
  // written first and the source is shrunk or deleted LAST, which makes the same
  // failure an OVER-COUNT: the source is still whole beside some already-written
  // pieces, and because the piece ids are deterministic the client's retry
  // re-lands on them and converges. The replay stamp goes on in that final write
  // for the same reason: stamped early, a failed run would answer the retry
  // "duplicate" and report success over lost time.
  async function reallocateTimeEntry({ entry, allocations, actor = "dcc", userId = null, workspaceId = null, actionId = null }) {
    ...
    // STARTED IS NOT SETTLED. The operation id rides on the row like startWork's does,
    // but a reallocation is several writes and the stamp used to be armed while three of
    // them were still outstanding (the source delete and both projections). A failure
    // after it was then permanent, because the retry read the stamp and reported
    // success: a whole move whose delete threw left the source LIVE and full beside the
    // moved row, inventing an hour rather than over-counting one. So the stamp only
    // means "this operation began", `reallocationSettledAt` means it finished, and an
    // unsettled replay RESUMES the tail off the prior totals stored with the stamp
    // instead of being answered.
    if (actionId && props.reallocationOperationId === actionId) {
      return resumeReallocation({ entry, actionId, actor, userId, workspaceId });
    }
    const window = entryWindow(entry, timeZone);
    const total = allocations.reduce((sum, part) => sum + Math.round(Number(part.durSec) || 0), 0);
    // Belt to planAllocations' braces. The store must not be the layer that
    // trusts the caller conserved the time, because it is the layer that writes it.
    if (!window.durSec || total !== window.durSec) throw new Error("Reallocation does not conserve tracked time");

    const originId = String(props.blockId || "");
    const fence = entry.workspace_id || workspaceId || null;
    // ABSENT IS NOT "MINE". The repo's usual fence is written
    // `if (a && b && a !== b) deny`, and db.js createBlock says in as many words that it
    // therefore PASSES on a row whose workspace is absent. `properties.blockId` is
    // client-writable, so without the fallback an unfenced legacy task could still be
    // named as the origin and written. With no tenant on the row, the only other
    // identity it has is its owner.
    const ownerId = entry.user_id == null ? null : String(entry.user_id);
    ...
    // Seeded on the OPERATION when there is one. The client mints one actionId per plan,
    // so the actionId IS the plan's identity, and it is stable across retries. A seed
    // built from the resolved destination ids is not: a `newTask` piece creates a fresh
    // task on every attempt, so after a partial failure the retry derived DIFFERENT row
    // ids, left the first attempt's rows orphaned, and permanently inflated the ledger
    // instead of converging on it.
    const seed = actionId || allocations.map(part => `${part.durSec}>${part.task.id}`).join("|");
    const at = new Date().toISOString();
    const keepIndex = allocations.findIndex(part => originId && String(part.task.id) === originId);

    // One day_root lookup per DISTINCT date, not per row. db.createItineraryTasks
    // states the same rule ("ensure each distinct day root once up front") and
    // createItineraryTask's own ensureRoot flag exists to express it.
    const dayRoots = new Map();
    async function dayRootFor(date) {
      if (!dayRoots.has(date)) {
        dayRoots.set(date, await blockDB.ensureDayRoot(date, entry.user_id || userId || null, fence));
      }
    ...
    // uses so a session that runs past local midnight becomes one row per local day,
    // each with the date its own clock window belongs to. Reallocation has the same
    // problem: pinning every piece to the SOURCE row's date produces a row whose `end`
    // precedes its `start` and which Day Review draws on the wrong day, since it
    // positions fills from `start` and orders segments by `date`. Reachable input, not
    // hypothetical: finalizeTiming writes one unsplit timer row for a hourglass that
    // spanned midnight, and Day Review's manual editor supports it on purpose
    // (`if (eMin <= sMin) eMin += 1440`).
    const planned = [];
    let offset = 0;
    for (let i = 0; i < allocations.length; i++) {
      const part = allocations[i];
      ...
      const spans = startMs == null
        ? [{ date: entry.date, startMs: null, endMs: null, durSec: part.durSec }]
      ...
      for (let j = 0; j < spans.length; j++) {
        ...
        if (span.startMs != null) {
          ...
          // spreading the old props: an ISO startedAt we invented here would survive
          // that edit, and entryWindow trusts startedAt first, so the next move of
          // that row would position its pieces at the old hour with the new length.
          if (props.startedAt) {
            ...
            delete nextProps.endedAt;
            delete nextProps.logicalSessionStartedAt;
            delete nextProps.logicalSessionEndedAt;
          }
        } else {
          // No readable start means no clock at all. Carrying the source's start/end
          // over would give every piece the same window with a different length, and
          // Day Review would stack them on top of each other.
          for (const key of ["start", "end", "startedAt", "endedAt", "logicalSessionStartedAt", "logicalSessionEndedAt"]) delete nextProps[key];
        }
        if (allocations.length > 1 || spans.length > 1) nextProps.splitFromEntryId = entry.id;
        ...
        // user chose, and reopenWork hard-deletes rows by exactly that field, so
        // "20m to onboarding, the rest stays on the meeting" lost the rest.
        delete nextProps.estimated;
        delete nextProps.inferred;
        delete nextProps.inferenceReason;
        if (moved) {
          // A fresh session id, so no later writeLogicalSession can derive a row id
          // that collides with a piece now filed under a different task; its identity
          // guard would throw and take the pause with it.
          nextProps.workSessionId = `ws-realloc-${reallocatedRowId(entry, seed, `${i}.${j}`).slice(-24)}`;
        ...
        planned.push({
          ...
          id: i === keepIndex && j === 0 ? entry.id : reallocatedRowId(entry, seed, `${i}.${j}`),
    ...
    // ── snapshot what each task's segments accounted for BEFORE the writes ──
    // recomputeActualMinutes needs this to tell a projection apart from minutes no
    // row ever explained, so it can carry the latter across instead of deleting it.
    const touched = new Map();
    if (originId) touched.set(originId, null);
    for (const part of allocations) touched.set(String(part.task.id), part.task);
    const priorTotals = new Map();
    for (const [id, known] of touched) {
      const block = known || await blockDB.getBlock(id);
      if (!block || block.deleted_at) continue;
      if (!writableHere(block)) continue;
      priorTotals.set(id, sessionTotalMinutes(await getSessions(block, { workspaceId: block.workspace_id || fence })));
    }

    async function writeRow(row, extraProps) {
      const rowProps = extraProps ? { ...row.props, ...extraProps } : row.props;
      if (row.reusesSource) {
        ...
        // day's container, and that day's root might never be ensured at all.
        const dayChanged = String(row.date || "") !== String(entry.date || "");
        return blockDB.updateBlock(entry.id, dayChanged
          ? { properties: rowProps, date: row.date, parent_id: await dayRootFor(row.date) }
          : { properties: rowProps, date: row.date });
      }
      const existing = typeof blockDB.getBlockIncludingDeleted === "function"
        ? await blockDB.getBlockIncludingDeleted(row.id)
        : await blockDB.getBlock(row.id);
      if (existing) {
        // The guard writeLogicalSession already applies to its own derived ids. Ids are
        // client-suppliable in this app (POST /api/blocks accepts one), so a row sitting
        // at a derived id is not provably ours: overwriting it blindly would let a
        // planted row in another tenant absorb this segment's properties, and would
        // silently clobber an unrelated row on any collision.
        const sameWorkspace = String(existing.workspace_id || "") === String(fence || "");
        if (!sameWorkspace || existing.type !== "time_entry") throw new Error("Reallocated segment identity collision");
        if (existing.deleted_at && typeof blockDB.undeleteBlock === "function") await blockDB.undeleteBlock(row.id);
        return blockDB.updateBlock(row.id, { properties: rowProps, date: row.date });
      }
    ...
    // Everything that is NOT the source row first (see WRITE ORDER above).
    const entries = [];
    for (const row of planned) if (!row.reusesSource) entries.push(await writeRow(row));

    // Then the source row, carrying the replay stamp, once every piece above landed.
    const keepRow = planned.find(row => row.reusesSource) || null;
    // Everything a resume needs travels WITH the started stamp, because the snapshot it
    // depends on cannot be retaken once the pieces have landed.
    const startedStamp = actionId ? {
      reallocationOperationId: actionId,
      reallocationPriorTotals: Object.fromEntries(priorTotals),
      reallocationTouchedIds: [...touched.keys()],
      reallocationKeptSource: !!keepRow,
    } : null;
    if (keepRow) {
      entries.unshift(await writeRow(keepRow, startedStamp));
    } else {
      // Nothing stayed, so the source is double-counted until it goes. It is STAMPED
      // first and DELETED last (in settleReallocation): the route answers a replay off
      // this row, so a bare delete would make the retry of a succeeded move read as a
      // 404 and toast a failure for work that landed, and deleting before the
      // projections are settled would strand them with no row left to resume from.
      if (startedStamp) {
        await blockDB.updateBlock(entry.id, {
          properties: { ...props, ...startedStamp, reallocatedAt: at, reallocatedBy: actor, movedToEntryIds: entries.map(row => row.id) },
        });
      }
    }

    const tasks = await settleReallocation({
      entry, actionId, keptSource: !!keepRow, priorTotals, touchedIds: [...touched.keys()],
      fence, ownerId, workspaceId,
    });
    return { entries, tasks, originTaskId: originId || null, sourceEntryDeleted: !keepRow };
  }

  // The part of a reallocation that runs AFTER every segment row is written: re-project
  // both ends, record that the operation finished, and only then drop a source that
  // nothing kept. Split out because a replay has to be able to run exactly this much
  // again, from the stamped prior totals rather than from a snapshot it can no longer
  // take: once the pieces have landed, re-reading the origin's sessions would count the
  // moved minutes as unexplained history and re-inflate it.
  async function settleReallocation({ entry, actionId, keptSource, priorTotals, touchedIds, fence, ownerId, workspaceId }) {
    // The source row of a whole move is doomed but still live, so it is excluded from
    // every projection below rather than deleted first.
    const doomed = keptSource ? null : new Set([entry.id]);
    function writableHere(block) {
      if (block.workspace_id) return String(block.workspace_id) === String(fence || "");
      if (!block.user_id) return true;
      return ownerId != null && String(block.user_id) === ownerId;
    ...
    for (const id of touchedIds) {
      ...
      if (!block || block.deleted_at) continue;
      // `properties.blockId` on a time_entry is CLIENT-WRITABLE (schemas.blockItem is
      // passthrough with a free-form type, and block-store's logTimeEntry writes these
      // rows), so the origin can name a task in another workspace while the segment
      // `properties.blockId` on a time_entry is CLIENT-WRITABLE, so the origin can name a
      // task in another workspace while the segment itself passes its own ownership
      // check, and db.updateBlock carries no tenant predicate.
      if (!writableHere(block)) continue;
      const result = await recomputeActualMinutes({
        ...
        excludeEntryIds: doomed,
      });
      if (result) tasks.push(result);
    }
    // Settled BEFORE the delete, because db.updateBlock refuses a tombstone: a marker
    // written after it could never be written at all. If the delete then fails, a replay
    // sees a settled-but-live source and finishes the job (see the duplicate branch).
    if (actionId) {
      const current = await blockDB.getBlockIncludingDeleted(entry.id);
      if (current && !current.deleted_at) {
        await blockDB.updateBlock(entry.id, {
          properties: { ...(current.properties || {}), reallocationSettledAt: new Date().toISOString() },
        });
      }
    }
    if (!keptSource) {
      const current = await blockDB.getBlockIncludingDeleted(entry.id);
      if (current && !current.deleted_at) await blockDB.deleteBlock(entry.id);
    }
    return tasks;
  }

  // Finish an operation that began but never settled. Everything it needs was stamped on
  // the source row by the run that started it, so this never re-derives a snapshot.
  async function resumeReallocation({ entry, actionId, actor = "dcc", userId = null, workspaceId = null }) {
    const props = entry.properties || {};
    const keptSource = props.reallocationKeptSource !== false;
    if (props.reallocationSettledAt) {
      // Settled, but the delete is the one step that can still be outstanding, because it
      // has to run after the marker. Finish it rather than reporting a move whose source
      // is somehow still on the ledger.
      if (!keptSource && !entry.deleted_at) await blockDB.deleteBlock(entry.id);
      return {
        entries: [entry], tasks: [], originTaskId: String(props.blockId || "") || null,
        sourceEntryDeleted: !keptSource, duplicate: true,
      };
    }
    const fence = entry.workspace_id || workspaceId || null;
    const stored = props.reallocationPriorTotals || {};
    const priorTotals = new Map(Object.keys(stored).map(id => [id, stored[id]]));
    const touchedIds = props.reallocationTouchedIds && props.reallocationTouchedIds.length
      ? props.reallocationTouchedIds
      : [...priorTotals.keys()];
    const tasks = await settleReallocation({
      entry, actionId, keptSource, priorTotals, touchedIds, fence,
      ownerId: entry.user_id == null ? null : String(entry.user_id),
      workspaceId,
    });
    return {
      entries: [entry], tasks, originTaskId: String(props.blockId || "") || null,
      sourceEntryDeleted: !keptSource, resumed: true,
    };
  }
  ...
  return {
    ...
    recomputeActualMinutes, reallocateTimeEntry, resumeReallocation,
  };
}
