...
function createTaskTiming({ pool, blockDB, timeZone = "America/New_York" } = {}) {
  ...
  async function deleteTimerRow(block) {
    ...
    await pool.query(
      "DELETE FROM blocks WHERE id = $1 AND type = 'time_entry' AND ($2::text IS NULL OR workspace_id = $2)",
  ...
  async function reconcileTiming(blocks, { userId = null, workspaceId = null } = {}) {
    ...
    const candidates = blocks.filter(b => {
      ...
      return !!p.startedAt && (p.actualMinutes == null || p.actualMinutesFrom === DERIVED_FLAG);
    });
    ...
    for (const block of candidates) {
      try {
        ...
        if (!isBlockDone(block, overlay)) {
          if ((block.properties || {}).actualMinutesFrom !== DERIVED_FLAG) continue;
