...
function createTaskTiming({ pool, blockDB, timeZone = "America/New_York" } = {}) {
  ...
  async function recomputeActualMinutes({ block, workspaceId = null, priorSessionsTotal }) {
    if (!block || !block.id) return null;
    const sessions = await getSessions(block, { workspaceId });
  ...
  async function reallocateTimeEntry({ entry, allocations, actor = "dcc", userId = null, workspaceId = null, actionId = null }) {
    ...
    // Same idempotency idiom as startWork/pauseWork: the operation id rides on
    // the row, so a replay is recognised rather than re-applied.
    if (actionId && props.reallocationOperationId === actionId) {
      return { entries: [entry], tasks: [], originTaskId: String(props.blockId || "") || null, sourceEntryDeleted: false, duplicate: true };
    }
    ...
    for (const [id, known] of touched) {
      ...
      if (fence && block.workspace_id && block.workspace_id !== fence) continue;
    ...
    async function writeRow(row, extraProps) {
      ...
      if (row.reusesSource) return blockDB.updateBlock(entry.id, { properties: rowProps, date: row.date });
      const existing = typeof blockDB.getBlockIncludingDeleted === "function"
    ...
    if (keepRow) {
      entries.unshift(await writeRow(keepRow, actionId ? { reallocationOperationId: actionId } : null));
    } else {
      // Nothing stayed, so the source is now double-counted and has to go. It is
      // STAMPED before it is deleted: the route answers a replay off this row, and a
      // bare delete would make the retry of a succeeded whole move read as 404
      // "Tracked time not found" and toast a failure for work that landed.
      if (actionId) {
        await blockDB.updateBlock(entry.id, {
          properties: { ...props, reallocationOperationId: actionId, reallocatedAt: at, reallocatedBy: actor, movedToEntryIds: entries.map(row => row.id) },
        });
      }
      await blockDB.deleteBlock(entry.id);
    }

    const tasks = [];
    for (const [id, known] of touched) {
      let block = known;
      if (!block) block = await blockDB.getBlock(id);
      if (!block || block.deleted_at) continue;
      ...
      // itself passes its own ownership check. db.updateBlock carries no tenant
      // predicate, so without this fence a recompute here is a cross-tenant write.
      if (fence && block.workspace_id && block.workspace_id !== fence) continue;
