...
function createTaskTiming({ pool, blockDB, timeZone = "America/New_York" } = {}) {
  ...
  async function recomputeActualMinutes({ block, workspaceId = null }) {
    ...
    return { id: block.id, actualMinutes: total == null ? null : total, previousMinutes: before };
  }
  ...
  //  2. Deterministic ids make a double-submit safe. A replay either finds the
  //     source already shortened (planAllocations rejects the sum) or already
  //     gone (the route 404s). Time can never be double-counted.
  async function reallocateTimeEntry({ entry, allocations, actor = "dcc", userId = null, workspaceId = null, actionId = null }) {
    ...
    // the row, so a replay is recognised rather than re-applied. It matters more
    // here than there, because the piece that STAYS keeps the source id — so a
    // double-submit of "20m to A, the rest stays" would otherwise be accepted a
    // second time against the now-shorter source and split it again.
    if (actionId && (entry.properties || {}).reallocationOperationId === actionId) {
      return { entries: [entry], tasks: [], originTaskId: String((entry.properties || {}).blockId || "") || null, sourceEntryDeleted: false, duplicate: true };
    }
    ...
    for (let i = 0; i < allocations.length; i++) {
      ...
      const endMs = startMs == null ? null : startMs + part.durSec * 1000;
      const nextProps = { ...props };
      nextProps.blockId = part.task.id;
      nextProps.taskTitle = (part.task.properties || {}).title || props.taskTitle || "Task";
      nextProps.durSec = part.durSec;
      nextProps.reallocatedAt = at;
      nextProps.reallocatedBy = actor;
      if (actionId) nextProps.reallocationOperationId = actionId;
      if (startMs != null) {
        nextProps.startedAt = new Date(startMs).toISOString();
        nextProps.endedAt = new Date(endMs).toISOString();
        nextProps.start = hhmm(startMs);
        nextProps.end = hhmm(endMs);
      ...
      if (allocations.length > 1) nextProps.splitFromEntryId = entry.id;
      if (moved) {
        // A fresh session id, so no later writeLogicalSession can derive a row id
        // that collides with a piece now filed under a different task — its
        // identity guard would throw and take the pause with it.
        nextProps.workSessionId = `ws-realloc-${reallocatedRowId(entry, seed, i).slice(-24)}`;
        ...
        // An inferred window a human has re-attributed is no longer a guess — and
        // reopenWork DELETES inferred rows, so leaving the flag on would let a
        // later reopen of the DESTINATION silently erase time placed by hand.
      ...
      if (i === keepIndex) {
        const written = await blockDB.updateBlock(entry.id, { properties: nextProps });
        entries.push(written || { ...entry, properties: nextProps });
      } else {
        const id = reallocatedRowId(entry, seed, i);
        const existing = typeof blockDB.getBlockIncludingDeleted === "function"
          ? await blockDB.getBlockIncludingDeleted(id)
          : await blockDB.getBlock(id);
        if (existing && existing.deleted_at && typeof blockDB.undeleteBlock === "function") await blockDB.undeleteBlock(id);
        if (existing) {
          entries.push(await blockDB.updateBlock(id, { properties: nextProps }));
        } else {
          const parentId = await blockDB.ensureDayRoot(entry.date, entry.user_id || userId || null, entry.workspace_id || workspaceId || null);
          entries.push(await blockDB.createBlock({
            id, type: "time_entry", parent_id: parentId, date: entry.date, properties: nextProps,
            user_id: entry.user_id || userId || null,
            workspace_id: entry.workspace_id || workspaceId || null,
          }));
        }
      }
      offset += part.durSec;
    ...
    // Nothing stayed, so the source row is the one piece of the ledger that is
    // now double-counted. Deleted AFTER every piece is written: the inverse order
    // loses the time outright if a write below it fails.
    if (keepIndex === -1) await blockDB.deleteBlock(entry.id);
    ...
    for (const [id, known] of touched) {
      ...
      const result = await recomputeActualMinutes({ block, workspaceId: workspaceId || block.workspace_id || null });
      if (result) tasks.push(result);
    }
    return { entries, tasks, originTaskId: originId || null, sourceEntryDeleted: keepIndex === -1 };
  }
