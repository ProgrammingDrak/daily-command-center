...
const CLOSURE_SQL = `
  WITH cand AS (
    ...
    -- count(DISTINCT b2.id) over an IN, not count(*) over a JOIN. The join form counted
    -- PAIRS: with c candidate rows carrying the local_id and m live blocks carrying it,
    -- every group held c*m rows, so it equalled the intended m only when c was 1. And
    -- because the cand CTE deliberately has no deleted_at filter (a tombstoned row's local_id
    -- is still a useful way to FIND the credit), a live row plus a soft-deleted twin gave
    -- c=2, m=1 and a count of 2 — refusing a revoke for a task with exactly one live
    -- owner. That is the false refusal the deleted_at filter here was supposed to
    -- prevent: the credit would stay banked on an un-done task AND keep eating the
    -- re-completion through ON CONFLICT. Latent today (Slack is the only revoke caller
    -- and it always presents a row id, so c is 1), live the moment a browser un-complete
    -- presents the local_id spelling.
    SELECT COALESCE(max(n), 1) AS n FROM (
      SELECT count(DISTINCT b2.id) AS n
        FROM blocks b2
       WHERE b2.properties->>'local_id' IN (SELECT DISTINCT local_id FROM cand WHERE local_id IS NOT NULL)
         AND b2.workspace_id IS NOT DISTINCT FROM $1
         AND b2.type <> 'day_root'
         AND b2.deleted_at IS NULL
         AND (b2.date IS NOT DISTINCT FROM $4::date OR b2.date IS NULL)
       GROUP BY b2.properties->>'local_id'
    ) g
  )
