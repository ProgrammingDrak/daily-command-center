...
const route = (fn) => async (req, res) => {
  try {
    ...
    // Honor both status conventions: assertBlockOwnership et al. set `.statusCode`
    // (repo-dominant); resolveOwnerStrict sets `.status` (shared with dcc.js).
    // Reading only one would map a deliberate 400 to a 500.
    if (!res.headersSent) res.status(e.statusCode || e.status || 500).json({
      error: e.message,
      ...(e.publicCode ? { code: e.publicCode } : {}),
    });
  }
};
