...
(function () {
  ...
  function openForm(def, prefillTitle) {
    const a = A();
    if (!a || typeof DCC.modal !== "function") return;
    const formDef = def || a.normalizeDef({ properties: { title: prefillTitle || "", periodMinutes: 1440 } });
    let handle = null;
    const actions = [{
      label: def ? "Save" : "Add",
      kind: "primary",
      onClick: async () => {
        ...
        try {
          await (def ? a.update(def.id, patch) : a.create(patch));
          buildAnytime();
          return true;
        } catch (err) {
          DCC.toast(err && err.message ? err.message : "That did not save", "error");
          return false;
        }
      }
    }];
    if (def) {
      actions.unshift({
        ...
        onClick: async () => {
          try {
            await a.remove(def.id);
            buildAnytime();
            return true;
          } catch (err) {
            DCC.toast(err && err.message ? err.message : "That did not delete", "error");
            return false;
          }
        }
      });
  ...
  function onRowClick(ev) {
    ...
    if (!a) return;
    const rendered = _vm && _vm.rows.find(r => r.def.id === id);
    const def = rendered && rendered.def;
    if (!def) return;
    if (!a.onTodayView()) {
    ...
    if (act === "edit") { openForm(def); return; }
    ...
    else if (act === "chk" && rendered.progress.done) delta = def.target === 1 ? -1 : 0;
    if (!delta) return;
    a.check(def, delta);
    buildAnytime();
  }
  ...
  function wire() {
    ...
    // Delegate row controls from the document because every render replaces them.
    document.addEventListener("click", ev => {
      if (!ev.target.closest("#anytime-panel")) return;
      if (ev.target.closest(".anytime-panel-close")) { closePanel(true); return; }
      if (ev.target.closest(".anytime-panel-add")) { openForm(null); return; }
      onRowClick(ev);
    });
  ...
  // The dock and panel exist above the script tags. Wire them immediately.
  wire();
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", buildAnytime, { once: true });
  } else { buildAnytime(); }
  ...
  DCC.AnytimeDock = {
    build: buildAnytime,
    open: openFromNudge,
    openCreate: title => openForm(null, title),
    close: closePanel,
    toggle: togglePanel,
