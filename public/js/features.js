...
function toggleTrivialTask(id){
  if(window.USE_BLOCKSTORE&&window.USE_BLOCKSTORE.trivialTasks&&window.blockStore){
    ...
    if(block){
      ...
      window.blockStore.setTaskCompletion(id,newDone,{
        block:block,taskDate:block.date||null,completedAt:newDone?new Date().toISOString():null
      }).then(()=>{
        buildTrivialTasks();
        if(typeof buildSchedule==='function')buildSchedule();
      }).catch(e=>{if(typeof showToast==='function')showToast(e.message||"Completion was not saved","error");});
    }
    return;
  }
  const tasks=loadTrivialTasks();
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (delegatedBtn) delegatedBtn.addEventListener('click', function(e) {
    ...
    var task = taskForRepeatResponsibility(_addModalTaskId);
    closeAddModal();
    setTimeout(function() { openDelegatedFromTask({ title: (task && (task.title || task.text)) || '', durMin: (task && task.durMin) || 30, sourceTaskId: (task && task.id) || _addModalTaskId }); }, 0);
  });
...
function _renderSmallTaskSection(opts){
  ...
  active.forEach(t=>{
    ...
    if(delegated)delegated.addEventListener("click",e=>{e.stopPropagation();if(typeof openDelegatedFromTask==="function")openDelegatedFromTask({title:t.text,durMin:t.durMin||30,sourceTaskId:t.id})});
...
function buildTaskQueuePanel(){
  ...
  if(!backlogTasks.length){
    backlogPanel.innerHTML='<div class="tqp-empty">Task Library is empty.</div>';
  }else{
    const priOrder={High:0,Medium:1,Low:2};
