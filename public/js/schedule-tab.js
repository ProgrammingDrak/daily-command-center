...
function buildSchedule(){
  ...
  flattenSchedule(activeItems).forEach(node=>{
    ...
    // The old "Make subtask of…" + "Make repeat responsibility" + "Delegated /
    // Blocked" buttons are now a single trigger that fans out the same radial menu
    // used by the bottom "+" launcher (Subtask / Repeat / Delegate / Backlog).
    const trb=el.querySelector(".btn-task-radial");if(trb)trb.addEventListener("click",e=>{e.stopPropagation();if(typeof window.dccOpenCardRadial!=="function")return;window.dccOpenCardRadial(trb,[
      {label:"Make subtask of…",icon:'<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 4v10a4 4 0 0 0 4 4h12M15 13l5 5-5 5"/></svg>',action:()=>{if(typeof openMakeSubtaskOf==="function")openMakeSubtaskOf(ev.id,trb);}},
      {label:"Repeat responsibility",icon:'<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17 1l4 4-4 4"/><path d="M3 11V9a4 4 0 0 1 4-4h14"/><path d="M7 23l-4-4 4-4"/><path d="M21 13v2a4 4 0 0 1-4 4H3"/></svg>',action:()=>{if(typeof openRepeatResponsibilityFromTask==="function")openRepeatResponsibilityFromTask(ev);}},
      {label:"Delegated / Blocked",icon:'<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M16 11l2 2 4-4"/></svg>',action:()=>{if(typeof convertTaskToDelegated==="function")convertTaskToDelegated(ev.id);}},
      {label:"Move to backlog",icon:'<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 12h-6l-2 3h-4l-2-3H2"/><path d="M5.45 5.11L2 12v6a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-6l-3.45-6.89A2 2 0 0 0 16.76 4H7.24a2 2 0 0 0-1.79 1.11z"/></svg>',action:()=>{if(typeof moveTaskToBacklog==="function")moveTaskToBacklog(ev.id);}}
    ]);});
    ...
    el.querySelector(".card").addEventListener("click",e=>{if(e.target.closest(".chk")||e.target.closest(".chk-quick")||e.target.closest(".dbtn")||e.target.closest(".dbadge")||e.target.closest(".dur-popover")||e.target.closest(".grip")||e.target.closest(".pomo-btn")||e.target.closest(".notes-btn")||e.target.closest(".btn-meeting-auto")||e.target.closest(".btn-repeat-resp")||e.target.closest(".btn-task-radial")||e.target.closest(".dcc-card-radial-host")||e.target.closest(".btn-move-menu")||e.target.closest(".move-menu-popup")||e.target.closest(".btn-del-task")||e.target.closest(".btn-lock")||e.target.closest(".btn-bounty")||e.target.closest(".btn-add-menu")||e.target.closest(".add-menu-popup")||e.target.closest(".itinerary-reactions")||e.target.closest(".card-triv-section")||e.target.closest(".start-time")||e.target.closest(".ttl"))return;if(typeof openAddModal==="function")openAddModal(ev.id,ev.title);});
