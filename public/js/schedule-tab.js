...
function buildSchedule(){
  ...
  activeItems.forEach(ev=>{
    ...
    el.innerHTML=
      ...
      '<div class="card-wrap">'+
        ...
        '<div class="card'+(active?' card-active':'')+(isBounty?' card-bounty':'')+'">'+
          ...
          (!isMeeting(ev)?'<button class="btn-move-menu" data-move-id="'+ev.id+'" data-tooltip="Move to..."><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M13 6l6 6-6 6"/></svg></button>':'')+
    ...
    const maBtn=el.querySelector(".btn-meeting-auto");if(maBtn)maBtn.addEventListener("click",e=>{e.stopPropagation();const panel=el.querySelector(".detail-panel");if(panel&&!panel.classList.contains("open")){panel.classList.add("open");const chev=el.querySelector(".card > svg:last-child");if(chev)chev.style.transform="rotate(180deg)";}if(typeof refreshMeetingAutomationPanels==="function")refreshMeetingAutomationPanels(maBtn.dataset.meetingAutoId);});
    const mvBtn=el.querySelector(".btn-move-menu");if(mvBtn)mvBtn.addEventListener("click",e=>{e.stopPropagation();openMoveMenu(mvBtn.dataset.moveId,mvBtn);});
