...
function toggleSBMenu() {
  const drop = document.getElementById("sb-menu-drop");
  const isOpen = drop.style.display !== "none";
  drop.style.display = isOpen ? "none" : "block";
  if (!isOpen) {
    // Update status line
    const date = (__state && __state.date) || "unknown";
    const sb = window.__SECOND_BRAIN__ || {};
    const sbDays = Object.keys(sb).length;
    const statusEl = document.getElementById("sb-status");
    PaDB.listDates().then(dates => {
      statusEl.textContent = `${sbDays} days in file DB · ${dates.length} in IndexedDB`;
    }).catch(() => {
      statusEl.textContent = `${sbDays} days in file DB`;
    });
    // Close on outside click
    setTimeout(() => {
      document.addEventListener("click", function closeSB(e) {
        if (!document.getElementById("sb-menu-drop").contains(e.target) && e.target.id !== "sb-menu-btn") {
...
async function downloadBackup() {
  const allState = { days: {}, globals: collectGlobalState(), exportedAt: new Date().toISOString(), source: "manual-backup" };
  // Collect all date-specific localStorage keys
  for (let i = 0; i < localStorage.length; i++) {
    const key = localStorage.key(i);
    if (key && key.startsWith("pa-") && key.match(/\d{4}-\d{2}-\d{2}$/)) {
      const date = key.match(/(\d{4}-\d{2}-\d{2})$/)[1];
      if (!allState.days[date]) allState.days[date] = { date };
      const field = key.replace(/-\d{4}-\d{2}-\d{2}$/, "").replace("pa-", "").replace(/-/g, "_");
      try { allState.days[date][field] = JSON.parse(localStorage.getItem(key)); } catch(e) {}
    }
  }
  // Also merge IndexedDB data
  try {
    const idb = await PaDB.exportAll();
    for (const [date, state] of Object.entries(idb.days || {})) {
      allState.days[date] = { ...(allState.days[date] || {}), ...state };
    }
  } catch(e) {}
  ...
  a.href = url;
  a.download = "second-brain-backup-" + ((__state && __state.date) || "export") + ".json";
  a.click();
  URL.revokeObjectURL(url);
  document.getElementById("sb-menu-drop").style.display = "none";
}
...
function loadBackup(event) {
  ...
  reader.onload = async function(e) {
    try {
      ...
      let restored = 0;
      // Restore per-day state
      if (data.days) {
        for (const [date, state] of Object.entries(data.days)) {
          writeToLocalStorage(date, state);
          await PaDB.saveDate(date, state);
          restored++;
        }
      }
      // Restore globals
      if (data.globals) {
        writeGlobalsToLocalStorage(data.globals);
        await PaDB.saveGlobal("globals", data.globals);
      }
      alert("Restored " + restored + " days from backup. Reloading...");
      location.reload();
    } catch(err) {
