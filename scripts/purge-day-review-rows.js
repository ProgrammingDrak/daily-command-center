...
async function main() {
  ...
  for (const { row, key } of keys) {
    try {
      ...
      failures.push(`credit ${key}`);
    }
  }
  console.log(`credits revoked         ${revoked}  (${clawedBack} points debited)`);
  if (failures.length) {
    throw new Error(`Stopped before block removal after ${failures.length} credit failure(s)`);
  }
  ...
  for (const row of rows) {
    ...
    try {
      ...
      failures.push(`block ${row.id}`);
    }
  }
  console.log(`blocks soft-deleted     ${deleted}  (${alreadyGone} already tombstoned)`);
  if (failures.length) {
    throw new Error(`Stopped before state pruning after ${failures.length} block failure(s)`);
  }
  ...
  for (const d of dirtyStates) {
    try {
      ...
      failures.push(`state ${ymd(d.row.date)}/${d.row.workspace_id}`);
    }
  }
  console.log(`dcc_state rows pruned   ${pruned}`);
  if (failures.length) {
    throw new Error(`Purge incomplete after ${failures.length} state failure(s)`);
  }
