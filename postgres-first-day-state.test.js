...
function runBuildDay({ dbRow = null, dbThrows = false, file = null, suppressionBlocks = [], suppressionsThrow = false, userId = 1 } = {}) {
  const writes = [];
  const settingsAsked = [];
  const ctx = {
    console: { error: () => {}, warn: () => {} },
  ...
  return { call: () => vm.runInContext(`buildDayResponse("${DATE}", ${uid}, "ws-1")`, ctx), writes, settingsAsked };
}
