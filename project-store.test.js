...
test("dependency uniqueness applies only while an edge is active", () => {
  for (const file of ["pg-schema.js", "migrations/008_task_library_projects.sql"]) {
    const source = fs.readFileSync(path.join(__dirname, file), "utf8");
    const index = source.match(/CREATE UNIQUE INDEX IF NOT EXISTS idx_blocks_task_dependency_edge[\s\S]*?;/);
    assert.ok(index, `${file} must create the dependency edge index`);
    assert.match(index[0], /COALESCE\(properties->>'status', 'open'\) IN \('open', 'ready'\)/,
      `${file} must allow a closed dependency to be created again`);
  }
});
