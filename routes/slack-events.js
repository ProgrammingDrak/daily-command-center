  ...
  function forActor(actor) {
    ...
    async function handleDone(channel, ts, eventMs, actionId = null) {
      ...
      if (!rows.bookmark && !rows.delegate) {
        console.warn(`[slack-events] ✅ with no task for ${channel}:${ts} - ignored`);
        return;
      }
      ...
      if (rows.delegate) {
        ...
        const task = rows.bookmark || await ensureDelegateCompletionTask(rows.delegate, channel, ts);
    ...
    async function handleUndone(channel, ts, eventMs = Date.now(), actionId = null) {
      ...
      let task = bookmark;
      if (reopenDelegate) {
        const linkedRef = String(delegateProps.linkedBlockId || "").trim();
        task = (linkedRef && await blockDB.getBlock(linkedRef))
          || await findLiveTaskByKey(delegateCompletionKeyFor(channel, ts))
          || bookmark;
      }
