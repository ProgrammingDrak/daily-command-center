...
module.exports = function mountSyncRoutes(app, ctx) {
  const {
    blockDB, broadcast, DAY_STATE_FILE, getDayFilePath, getTodayStr, isValidDate,
  ...
  app.get("/api/sync/bootstrap", async (req, res, next) => {
    try {
      ...
      res.json(await syncStore.bootstrap({ workspaceId, date }));
  ...
  app.post("/api/ingest/day-state/v2", async (req, res) => {
    ...
    try {
      ...
      const known = typeof blockDB.getBlocksByKind === "function"
        ? await blockDB.getBlocksByKind(triageSuppressions.SUPPRESSION_KIND, workspaceId)
        : [];
      const knownIds = new Set(known.map(triageSuppressions.suppressionFromBlock).filter(Boolean).map((row) => row.triage_id));
      for (const item of resolvedEvents) {
        if (!item || !item.id || knownIds.has(String(item.id))) continue;
        const reason = String(item.resolved_reason || item.reason || "").toLowerCase();
        if (!["replied", "done", "deleted"].includes(reason)) continue;
        await blockDB.createBlock({
          ...
          properties: triageSuppressions.buildSuppressionProperties({
            ...
            note: reason === "replied" ? "Replied at source" : "",
    ...
    try {
      writeJSON(getDayFilePath(date), compact);
      writeJSON(DAY_STATE_FILE, compact);
    } catch (error) {
      console.error("[dcc-state v2] mirror failed:", error.message);
    }
